...
enum evseCmd { EVSE_CMD_NONE, EVSE_CMD_STATE, EVSE_CMD_WIFI, EVSE_CMD_SLEEP, EVSE_CMD_ENABLE,
                  ...
                  EVSE_CMD_GETTEMP, EVSE_CMD_GETENERGY, EVSE_CMD_GETSTATE, EVSE_CMD_GETLEVEL,
                  EVSE_CMD_SETLIMIT };
...
void zclOpenEvse_Init( byte task_id )
{
  ...
  osal_start_timerEx( zclOpenEvse_TaskID, OPENEVSE_POLL_EVSE_EVT, 1000 ); // 1 second for EVSE to boot
...
uint16 zclOpenEvse_event_loop( uint8 task_id, uint16 events )
{
  ...
  if (events & OPENEVSE_POLL_EVSE_EVT)
  {
    ...
    case 0: // State 0-9 initialization
      zclOpenEvse_EVSEWriteCmd(EVSE_CMD_GETLEVEL, 0);
      break;
    case 1:
      if (zclOpenEvse_backlight == LIGHT_OFF) // Restore backlight state if power cycled
      {
        zclOpenEvse_EVSEWriteCmd(EVSE_CMD_LCDOFF, 0);
      }
      break;
...
void zclOpenEvse_sendPower(void)
{
  ...
  zcl_SendReportCmd( OPENEVSE_ENDPOINT, &zclOpenEvse_DstAddr,
                     ...
                     ZCL_FRAME_SERVER_CLIENT_DIR, 1, zclOpenEvse_seqNum++ ); 
...
void zclOpenEvse_EVSEWriteCmd(uint8 command, uint8 numArgs, ...)
{
  ...
  if (command == EVSE_CMD_NONE) // NONE command means resend last one
  {
...
{
  if (zclOpenEvse_evseResendCtr++ > 10)
  {
...
void zclOpenEvse_UARTParse(char * rxData)
{
  ...
  case EVSE_CMD_GETSTATE:
    ...
    break;
  case EVSE_CMD_GETLEVEL:
    {
      char * level = strtok(&rxData[3], " ");
      if (!level)
      {
        zclOpenEvse_EVSEResend();
        return;
      ...
      zclOpenEvse_powerLevel = atoi(level);
    }
    break;
